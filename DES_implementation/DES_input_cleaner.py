...
#Arranges input nicely for a binary string
def clean_input_binary(list_of_chars):
    DES_input_blocks = []

    input_total_length = len(list_of_chars)
    bit_pointer = 0
    block_pointer = 0
    current_block = ""
    while bit_pointer < input_total_length:
        if block_pointer < 64:
            current_block += list_of_chars[bit_pointer]
            block_pointer += 1
            bit_pointer += 1
        else: 
            block_pointer = 0
